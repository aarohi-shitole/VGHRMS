...
import com.techvg.hrms.service.mapper.LoginUserMapper;
...
public class UserService {
	...
	public List<String> getAuthorities() {
		return authorityRepository.findAll().stream().map(TechvgRole::getName).collect(Collectors.toList());
	...
	private User syncUserWithIdP(Map<String, Object> details, User user) {
		...
		Collection<String> userAuthorities = user.getTechvgRoles().stream().map(TechvgRole::getName)
				.collect(Collectors.toList());
		for (String authority : userAuthorities) {
			if (!dbAuthorities.contains(authority)) {
				log.debug("Saving authority '{}' in local database", authority);
				TechvgRole authorityToSave = new TechvgRole();
...
//for role and permissions	
	...
	public AdminUserDTO getUserFromAuthentication(AbstractAuthenticationToken authToken) {
		...
		user.setTechvgRoles(authToken.getAuthorities().stream().map(GrantedAuthority::getAuthority).map(authority -> {
			TechvgRole auth = new TechvgRole();
	...
	private static User getUser(Map<String, Object> details) {
		...
		if (details.get("companyId") != null) {
			user.setCompanyId((Long) details.get("companyId"));
		}
		if (details.get("employeeId") != null) {
			user.setEmployeeId((Long) details.get("employeeId"));
		}
