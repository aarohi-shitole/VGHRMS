 ...
 * io.jsonwebtoken.*; import io.jsonwebtoken.io.Decoders; import
 * io.jsonwebtoken.security.Keys; import
 * io.jsonwebtoken.security.SignatureException; import
 * java.nio.charset.StandardCharsets; import java.security.Key; import
 * java.util.*; import java.util.stream.Collectors; import org.slf4j.Logger;
 ...
 * SecurityMetersService securityMetersService) { byte[] keyBytes; String secret
 * =
 * jHipsterProperties.getSecurity().getAuthentication().getJwt().getBase64Secret
 * (); if (!ObjectUtils.isEmpty(secret)) {
 * log.debug("Using a Base64-encoded JWT secret key"); keyBytes =
 * Decoders.BASE64.decode(secret); } else { log.warn(
 * "Warning: the JWT key used is not Base64-encoded. " +
 * "We recommend using the `jhipster.security.authentication.jwt.base64-secret` key for optimum security."
 * ); secret =
 * jHipsterProperties.getSecurity().getAuthentication().getJwt().getSecret();
 * keyBytes = secret.getBytes(StandardCharsets.UTF_8); } key =
 * Keys.hmacShaKeyFor(keyBytes); jwtParser =
 * Jwts.parserBuilder().setSigningKey(key).build();
 * this.tokenValidityInMilliseconds = 1000 *
 * jHipsterProperties.getSecurity().getAuthentication().getJwt().
 * getTokenValidityInSeconds(); this.tokenValidityInMillisecondsForRememberMe =
 ...
 * 
 * long now = (new Date()).getTime(); Date validity; if (rememberMe) { validity
 * = new Date(now + this.tokenValidityInMillisecondsForRememberMe); } else {
 ...
 * .stream(claims.get(AUTHORITIES_KEY).toString().split(",")) .filter(auth ->
 * !auth.trim().isEmpty()) .map(SimpleGrantedAuthority::new)
