...
import com.techvg.hrms.service.ReportingQueryService;
import com.techvg.hrms.service.ReportingService;
import com.techvg.hrms.service.criteria.ReportingCriteria;
...
/**
 * REST controller for managing {@link com.techvg.hrms.domain.Reporting}.
 */
@RestController
@RequestMapping("/api")
public class ReportingResource {
	...
	public ReportingResource(ReportingService reportingService, ReportingRepository reportingRepository,
	 ...
	 *         body the new reportingDTO, or with status {@code 400 (Bad Request)}
	 *         if the reporting has already an ID.
	...
	public ResponseEntity<ReportingDTO> createReporting(@RequestBody ReportingDTO reportingDTO)
		...
		return ResponseEntity
				.created(new URI("/api/reportings/" + result.getId())).headers(HeaderUtil
	 ...
	 *         the updated reportingDTO, or with status {@code 400 (Bad Request)} if
	 *         the reportingDTO is not valid, or with status
	 *         {@code 500 (Internal Server Error)} if the reportingDTO couldn't be
	 *         updated.
	 * @throws URISyntaxException if the Location URI syntax is incorrect.
	 */
	@PutMapping("/reportings/{id}")
	public ResponseEntity<ReportingDTO> updateReporting(@PathVariable(value = "id", required = false) final Long id,
			@RequestBody ReportingDTO reportingDTO) throws URISyntaxException {
	...
	/**
	 * {@code PATCH  /reportings/:id} : Partial updates given fields of an existing
	 * reporting, field will ignore if it is null
	 *
	 * @param id           the id of the reportingDTO to save.
	 * @param reportingDTO the reportingDTO to update.
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the updated reportingDTO, or with status {@code 400 (Bad Request)} if
	 *         the reportingDTO is not valid, or with status {@code 404 (Not Found)}
	 *         if the reportingDTO is not found, or with status
	 *         {@code 500 (Internal Server Error)} if the reportingDTO couldn't be
	 *         updated.
	 * @throws URISyntaxException if the Location URI syntax is incorrect.
	 */
	...
	public ResponseEntity<ReportingDTO> partialUpdateReporting(
			@PathVariable(value = "id", required = false) final Long id, @RequestBody ReportingDTO reportingDTO)
			throws URISyntaxException {
		...
		return ResponseUtil.wrapOrNotFound(result, HeaderUtil.createEntityUpdateAlert(applicationName, true,
				ENTITY_NAME, reportingDTO.getId().toString()));
	}
	 ...
	 *         of reportings in body.
	 */
	@GetMapping("/reportings")
	public ResponseEntity<List<ReportingDTO>> getAllReportings(ReportingCriteria criteria,
	 ...
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the reportingDTO, or with status {@code 404 (Not Found)}.
	 */
