...
import com.techvg.hrms.service.BanksDetailsQueryService;
import com.techvg.hrms.service.BanksDetailsService;
import com.techvg.hrms.service.criteria.BanksDetailsCriteria;
...
/**
 * REST controller for managing {@link com.techvg.hrms.domain.BanksDetails}.
 */
@RestController
@RequestMapping("/api")
public class BanksDetailsResource {
	...
	public BanksDetailsResource(BanksDetailsService banksDetailsService, BanksDetailsRepository banksDetailsRepository,
	 ...
	 *         body the new banksDetailsDTO, or with status
	 *         {@code 400 (Bad Request)} if the banksDetails has already an ID.
	 * @throws URISyntaxException if the Location URI syntax is incorrect.
	 */
	@PostMapping("/banks-details")
	public ResponseEntity<BanksDetailsDTO> createBanksDetails(@RequestBody BanksDetailsDTO banksDetailsDTO)
		...
		return ResponseEntity
				.created(new URI("/api/banks-details/" + result.getId())).headers(HeaderUtil
	 ...
	 *         the updated banksDetailsDTO, or with status {@code 400 (Bad Request)}
	 *         if the banksDetailsDTO is not valid, or with status
	 *         {@code 500 (Internal Server Error)} if the banksDetailsDTO couldn't
	 *         be updated.
	 * @throws URISyntaxException if the Location URI syntax is incorrect.
	 */
	@PutMapping("/banks-details/{id}")
	public ResponseEntity<BanksDetailsDTO> updateBanksDetails(
			@PathVariable(value = "id", required = false) final Long id, @RequestBody BanksDetailsDTO banksDetailsDTO)
			throws URISyntaxException {
		...
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME,
				banksDetailsDTO.getId().toString())).body(result);
	}

	/**
	 * {@code PATCH  /banks-details/:id} : Partial updates given fields of an
	 * existing banksDetails, field will ignore if it is null
	 *
	 * @param id              the id of the banksDetailsDTO to save.
	 * @param banksDetailsDTO the banksDetailsDTO to update.
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the updated banksDetailsDTO, or with status {@code 400 (Bad Request)}
	 *         if the banksDetailsDTO is not valid, or with status
	 *         {@code 404 (Not Found)} if the banksDetailsDTO is not found, or with
	 *         status {@code 500 (Internal Server Error)} if the banksDetailsDTO
	 *         couldn't be updated.
	 * @throws URISyntaxException if the Location URI syntax is incorrect.
	 */
	...
	public ResponseEntity<BanksDetailsDTO> partialUpdateBanksDetails(
		...
		return ResponseUtil.wrapOrNotFound(result, HeaderUtil.createEntityUpdateAlert(applicationName, true,
				ENTITY_NAME, banksDetailsDTO.getId().toString()));
	}
	 ...
	 *         of banksDetails in body.
	 */
	@GetMapping("/banks-details")
	public ResponseEntity<List<BanksDetailsDTO>> getAllBanksDetails(BanksDetailsCriteria criteria,
	 ...
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the banksDetailsDTO, or with status {@code 404 (Not Found)}.
	 */
