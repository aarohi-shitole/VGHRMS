...
public class AddressResource {
	...
	public ResponseEntity<AddressDTO> updateAddress(@PathVariable(value = "id", required = false) final Long id,
			@RequestBody AddressDTO addressDTO) throws URISyntaxException {
	 ...
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the updated addressDTO, or with status {@code 400 (Bad Request)} if
	 *         the addressDTO is not valid, or with status {@code 404 (Not Found)}
	 *         if the addressDTO is not found, or with status
	 *         {@code 500 (Internal Server Error)} if the addressDTO couldn't be
	 *         updated.
	 * @throws URISyntaxException if the Location URI syntax is incorrect.
	 */
	@PatchMapping(value = "/addresses/{id}", consumes = { "application/json", "application/merge-patch+json" })
	public ResponseEntity<AddressDTO> partialUpdateAddress(@PathVariable(value = "id", required = false) final Long id,
			@RequestBody AddressDTO addressDTO) throws URISyntaxException {
