...
import com.techvg.hrms.service.PersonalIdQueryService;
import com.techvg.hrms.service.PersonalIdService;
import com.techvg.hrms.service.criteria.PersonalIdCriteria;
...
/**
 * REST controller for managing {@link com.techvg.hrms.domain.PersonalId}.
 */
@RestController
@RequestMapping("/api")
public class PersonalIdResource {
	...
	public PersonalIdResource(PersonalIdService personalIdService, PersonalIdRepository personalIdRepository,
	 ...
	 *         body the new personalIdDTO, or with status {@code 400 (Bad Request)}
	 *         if the personalId has already an ID.
	 * @throws URISyntaxException if the Location URI syntax is incorrect.
	 */
	@PostMapping("/personal-ids")
	public ResponseEntity<PersonalIdDTO> createPersonalId(@RequestBody PersonalIdDTO personalIdDTO)
		...
		return ResponseEntity
				.created(new URI("/api/personal-ids/" + result.getId())).headers(HeaderUtil
	 ...
	 *         the updated personalIdDTO, or with status {@code 400 (Bad Request)}
	 *         if the personalIdDTO is not valid, or with status
	 *         {@code 500 (Internal Server Error)} if the personalIdDTO couldn't be
	 *         updated.
	 * @throws URISyntaxException if the Location URI syntax is incorrect.
	 */
	@PutMapping("/personal-ids/{id}")
	public ResponseEntity<PersonalIdDTO> updatePersonalId(@PathVariable(value = "id", required = false) final Long id,
			@RequestBody PersonalIdDTO personalIdDTO) throws URISyntaxException {
		...
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME,
				personalIdDTO.getId().toString())).body(result);
	}

	/**
	 * {@code PATCH  /personal-ids/:id} : Partial updates given fields of an
	 * existing personalId, field will ignore if it is null
	 *
	 * @param id            the id of the personalIdDTO to save.
	 * @param personalIdDTO the personalIdDTO to update.
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the updated personalIdDTO, or with status {@code 400 (Bad Request)}
	 *         if the personalIdDTO is not valid, or with status
	 *         {@code 404 (Not Found)} if the personalIdDTO is not found, or with
	 *         status {@code 500 (Internal Server Error)} if the personalIdDTO
	 *         couldn't be updated.
	 * @throws URISyntaxException if the Location URI syntax is incorrect.
	 */
	...
	public ResponseEntity<PersonalIdDTO> partialUpdatePersonalId(
			@PathVariable(value = "id", required = false) final Long id, @RequestBody PersonalIdDTO personalIdDTO)
			throws URISyntaxException {
		...
		return ResponseUtil.wrapOrNotFound(result, HeaderUtil.createEntityUpdateAlert(applicationName, true,
				ENTITY_NAME, personalIdDTO.getId().toString()));
	}
	 ...
	 *         of personalIds in body.
	 */
	@GetMapping("/personal-ids")
	public ResponseEntity<List<PersonalIdDTO>> getAllPersonalIds(PersonalIdCriteria criteria,
	 ...
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the personalIdDTO, or with status {@code 404 (Not Found)}.
	 */
